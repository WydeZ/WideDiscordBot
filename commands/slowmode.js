...
module.exports = {
	...
	aliases: ['slow', 'sm'],
	usage: 'slowmode <number> | slowmode off | slowmode <#channel> <number>',
	cooldown: 1,
...
if (!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send("You do not have the right permission: ```MANAGE MESSAGES```")
            ...
            if (args[1] === "off") {
                ...
                if (!args[1]) return message.channel.send('How many seconds do you want the slowmode to be? | Usage: slowmode <number/off> or slowmode <#channel> <number> | Example: slowmode 5')
    
            if (!slowchannel) {
                let hi = args[1]
                if (isNaN(hi)) return message.channel.send('It needs to be a number | Usage: slowmode <number/off> or slowmode <#channel> <number> | Example: slowmode 5')
                ...
                message.channel.setRateLimitPerUser(hi).catch((_err) => {
                    return message.channel.send('I was unable to fulfill your request | Try making the number a whole number | Bug maybe? Report it using the !bug command')
                })
